...
app.get("/groceries/index/:id", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT * FROM grocery_items WHERE grocery_id = $1",
      [id]
    );
...
app.get("/groceries/:category", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT * FROM grocery_items WHERE LOWER(category) = LOWER($1)",
      [category]
    );
    const groceries = result.rows;
    res.json(groceries);
    ...
    res
      .status(500)
      .send(`Unable to Recieve Grocery Items with category: ${category}`);
  }
});
...
app.get("/cartpage/:id", async (req, res) => {
  try {
    ...
    const query = `SELECT user_id, grocery_id, quantity, item_cost 
        FROM shopping_cart 
        WHERE user_id = $1`;
    const values = [id];
